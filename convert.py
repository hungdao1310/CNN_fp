...
def convert_file_values(file_path, q_bits, i_bits):
  max_value = float('-inf')
  index = 0
  max_position = None
...
python_path = 'ofm.txt'
file_path = 'ofm_rtl.txt'  # Replace with the path to your file
q_bits = 13  # Replace with the number of quantization bits
i_bits = 11  # Replace with the number of integer bits

value, position = convert_file_values(python_path, q_bits, i_bits)
max_value, max_position = convert_file_values(file_path, q_bits, i_bits)
with open(label_path, 'r') as file:
  label = int(file.readline().strip())
print(f"Label: {label}")
print(f"Detect in python: {position}")
print(f"Detect in RTL   : {max_position}")
